...
module BP3D.Floorplanner {
  ...
  // corner config
  const cornerRadiusCm = 0
  const cornerRadiusHoverCm = 12
  ...
  export class FloorplannerView {
    ...
    constructor(private floorplan: Model.Floorplan, private viewmodel: Floorplanner, private canvas: string) {
      this.canvasElement = <HTMLCanvasElement>$(canvas)[0];
    ...
    private drawWall(wall: Model.Wall) {
      ...
      this.drawLine(
        ...
        (hover ? wallWidthHoverCm  : wallWidthCm) * this.viewmodel.pixelsPerCm,
        color
      );
    ...
    private drawCorner(corner: Model.Corner) {
      ...
      this.drawCircle(
        ...
        (hover ? cornerRadiusHoverCm : cornerRadiusCm) * this.viewmodel.pixelsPerCm,
        color
      );
    }
    ...
    private drawTarget(x: number, y: number, lastNode) {
      this.drawCircle(
        ...
        cornerRadiusHoverCm * this.viewmodel.pixelsPerCm,
        cornerColorHover
      );
      if (this.viewmodel.lastNode) {
        this.drawLine(
          ...
          this.viewmodel.convertX(x),
          this.viewmodel.convertY(y),
          wallWidthHoverCm *  this.viewmodel.pixelsPerCm,
          wallColorHover
        );
      }
    }
    ...
    private calculateGridOffset(n,gridSpacing) {
      if (n >= 0) {
