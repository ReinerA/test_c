	...
	export var Controller = function (three, model, camera, element, controls, hud) {
		...
		function checkWallsAndFloorsAndRoof(event?) {
			...
			if (state == states.UNSELECTED && mouseoverObject == null) {
				...
				var roofIntersects = scope.getIntersections(
					mouse, roofPlanes, false);
		...
		this.onItemMoved = function(event)
		{
			...
			mouseMoved = false;
			mouseDown = true;
			if (selectedObject)switchState(states.DRAGGING);	
		}
		...
		this.onItemRotate = function(event)
		{
			...
			mouseMoved = false;
			mouseDown = true;
			if (selectedObject)switchState(states.ROTATING);	
		}
