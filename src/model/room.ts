...
module BP3D.Model {
	...
	*/
	export class Room {
		...
		/** */
		public roofChangeCallbacks = $.Callbacks();
		...
		*  ordered CCW
		*/
		constructor(private floorplan: Floorplan, public corners: Corner[],private defaultRoomTexture : Items.TextureData,private defaultRoofTexture : Items.TextureData) {
			this.updateWalls();
		...
		public fireOnRoofChange(callback) {
		...
		*/
		...
		public setRoofTexture(textureUrl: string, textureStretch, textureScale: number,item_Id : string) {
			var uuid = this.getUuid();
		...
		private generateRoofPlane() {
			...
			this.roofPlane = new THREE.Mesh(geometry,
				new THREE.MeshBasicMaterial({
						side: THREE.DoubleSide
			...
			(<any>this.roofPlane).roof = this.roof; // js monkey patch
		}
