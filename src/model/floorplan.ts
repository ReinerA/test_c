...
module BP3D.Model {
  ...
  export class Floorplan {
    ...
    */
    ...
    constructor(private defaultWallTexture: Items.TextureData,  private defaultRoomTexture : Items.TextureData,  private defaultRoofTexture : Items.TextureData) {
   ...
   private roofPlanes(): THREE.Mesh[] {
    ...
    public newWall(start: Corner, end: Corner): Wall {
      ...
      var wall = new Wall(start, end,this.defaultWallTexture);
    ...
    public overlappedCornerOrWall(x: number, y: number, cornerRadius: number, wallWidth: number){
    var result = null;
    
    var mn = cornerRadius;
    
    for (var i = 0; i < this.corners.length; i++) {
    	let corners = this.corners[i];
    	let dist = corners.distanceFrom(x, y);
       ...
       mn = Math.min(mn,wallWidth); 
    	...
    	let dist = wall.distanceFrom(x, y);
    ...
    public setFloorTexture(uuid: string, url: string, scale: number,item_Id : string) {
    ...
    public getRoofTexture(uuid: string) {
    ...
    public setRoofTexture(uuid: string, url: string, scale: number,item_Id : string) {
    ...
    public update() {
      ...
      roomCorners.forEach((corners) => {
        scope.rooms.push(new Room(scope, corners,scope.defaultRoomTexture,scope.defaultRoofTexture));
      });
